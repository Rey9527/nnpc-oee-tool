...
function airtableUrl(path, params) {
  ...
  const tableName = registerTableName();
...
function registerTableName() {
  const tableName = String(process.env.AIRTABLE_REGISTER_TABLE_NAME || process.env.AIRTABLE_TABLE_NAME || '').trim();
  if (!tableName) {
    throw new Error('Missing environment variable: AIRTABLE_REGISTER_TABLE_NAME or AIRTABLE_TABLE_NAME');
  }
  return tableName;
}
...
async function airtableFetch(path, options, params) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      throw new Error(`Airtable 404: 找不到會員 Register 表。請確認 AIRTABLE_BASE_ID=${baseId}，AIRTABLE_REGISTER_TABLE_NAME 或 AIRTABLE_TABLE_NAME="${tableName}" 是否完全正確。`);
    }
