...
function certificateTableRef() {
  return String(process.env.AIRTABLE_CERTIFICATES_TABLE_ID || process.env.AIRTABLE_CERTIFICATES_TABLE_NAME || 'Certificates').trim();
}

function airtableUrl(path, params) {
  const base = requireEnv('AIRTABLE_BASE_ID').trim();
  const table = encodeURIComponent(certificateTableRef());
...
async function airtableFetch(path, options, params) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      throw new Error(`Airtable 404: 找不到 Certificates 表。請確認 AIRTABLE_BASE_ID 與 AIRTABLE_CERTIFICATES_TABLE_ID 或 AIRTABLE_CERTIFICATES_TABLE_NAME="${certificateTableRef()}"。`);
    }
    if (/authentication required/i.test(detail)) {
      throw new Error('Airtable token 無效或沒有送到 Airtable。請確認 Vercel 的 AIRTABLE_TOKEN 是 pat... 開頭的完整 Personal Access Token，不是 Token ID，且已重新部署 Production。');
    }
    if (/invalid permissions|model was not found|required permissions|ids are correct/i.test(detail)) {
      throw new Error(`Airtable 權限或資料表設定錯誤。請確認 Personal Access Token 已加入目前 Base，scope 至少包含 data.records:read 與 data.records:write，且 AIRTABLE_CERTIFICATES_TABLE_ID 或 AIRTABLE_CERTIFICATES_TABLE_NAME="${certificateTableRef()}" 是真正的 Certificates 表 ID/名稱。原始錯誤：${detail}`);
    }
