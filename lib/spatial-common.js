...
function classify(record) {
  ...
  const types = [];
  if (includesAny(record.needs, ['人員定位','安全管理','資產','設備追蹤','車輛','搬運','巡檢','派工','異常','回溯','導航','location','positioning','safety','asset','equipment','vehicle','flow','inspection','dispatch','incident','audit','navigation','ตำแหน่ง','ความปลอดภัย','ทรัพย์สิน','อุปกรณ์','ยานพาหนะ','ตรวจงาน','มอบหมาย','เหตุการณ์','นำทาง'])) types.push('Location & movement');
  if (includesAny(record.needs, ['資產','設備','工具','醫療設備','模具','治具','asset','equipment','tool','medical','mold','fixture','อุปกรณ์','เครื่องมือ','แม่พิมพ์'])) types.push('Device operations');
  if (includesAny(record.needs, ['空間使用率','人流','停留','熱區','閒置','space utilization','people flow','dwell','hot zone','idle','การใช้พื้นที่','เวลาพำนัก','พื้นที่ว่าง'])) types.push('Spatial analysis');
  if (includesAny(record.needs, ['導航','路線','route guidance','navigation','นำทาง','เส้นทาง'])) types.push('Navigation');
  if (includesAny(record.needs, ['巡檢','派工','任務','dispatch','task','inspection','ตรวจงาน','มอบหมาย','ภารกิจ'])) types.push('Dispatch');
  if (includesAny(record.needs, ['異常','回溯','事故','遺失','audit','incident','history','loss','เหตุการณ์','สูญหาย'])) types.push('History / audit trail');
...
function estimateAssessment(record) {
  ...
  const searchEvents = includesAny(record.needs, ['資產','設備','工具','搜尋','asset','equipment','tool','search','ทรัพย์สิน','อุปกรณ์','ค้นหา']) ? mapEvents * 0.25 : 0;
  const routeEvents = includesAny(record.needs, ['導航','路線','派工','任務','navigation','route','dispatch','task','นำทาง','เส้นทาง','มอบหมาย','ภารกิจ']) ? assets * days * 1.5 : 0;
  ...
  const deviceUsd =
    (includesAny(record.needs, ['資產','設備','工具','醫療設備','模具','治具','asset','equipment','tool','medical','device','อุปกรณ์','เครื่องมือ','แม่พิมพ์']) ? assets * 0.1 : 0) +
    messages * 0.00015 +
    (includesAny(record.needs, ['人員定位','安全管理','資產','車輛','搬運','巡檢','派工','異常','導航','location','positioning','asset','vehicle','movement','inspection','dispatch','incident','navigation','ตำแหน่ง','ทรัพย์สิน','ยานพาหนะ','ตรวจงาน','เหตุการณ์','นำทาง']) ? messages * 0.0015 : 0);

  let analysisUsd = 0;
  if (includesAny(record.needs, ['空間使用率','人流','停留','熱區','閒置','分析','space utilization','analysis','people flow','dwell','hot zone','idle','การใช้พื้นที่','วิเคราะห์'])) {
  ...
  const twdThbRate = Number(process.env.SPATIAL_TWD_THB_RATE || 1.05);
  const mapCostTwd = mapUsd * fx;
  const deviceCostTwd = deviceUsd * fx;
  const analysisCostTwd = analysisUsd * fx;
  const platformCostTwd = mapCostTwd + deviceCostTwd + analysisCostTwd;
  const accuracy = String(record.accuracy || '').toLowerCase();
  const accuracyComplexity = /精準|precise|แม่นยำ/.test(accuracy)
    ? 80000
    : /公尺|meters|เมตร/.test(accuracy) ? 30000 : /區|zone/.test(accuracy) ? 10000 : 0;
  const complexity =
    record.needs.length * 6000 +
    sites * 2500 +
    accuracyComplexity +
    (/existing|擴充|ขยาย/.test(stage) ? 60000 : 0);
  const setupFeeTwd = Math.max(80000, Math.round(complexity / 1000) * 1000);
  const opsBaseTwd = classification.scale === 'Small' ? 18000 : classification.scale === 'Medium' ? 45000 : 90000;
  const monthlyInternalTwd = platformCostTwd + opsBaseTwd;
  const toThb = (twd) => Math.round(twd * twdThbRate);
  const setupFee = Math.ceil(toThb(setupFeeTwd) / 1000) * 1000;
  const suggestedMonthly = Math.ceil((toThb(monthlyInternalTwd) / 0.55) / 1000) * 1000;
  return {
    classification,
    usage: { map_events: mapEvents, search_events: searchEvents, route_events: routeEvents, messages },
    quote_currency: 'THB',
    exchange_rates: { usd_to_twd: fx, twd_to_thb: twdThbRate },
    map_cost: toThb(mapCostTwd),
    device_cost: toThb(deviceCostTwd),
    analysis_cost: toThb(analysisCostTwd),
    platform_cost: toThb(platformCostTwd),
    operations_cost: toThb(opsBaseTwd),
    monthly_internal: toThb(monthlyInternalTwd),
